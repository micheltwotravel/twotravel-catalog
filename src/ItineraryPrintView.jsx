...
function matchCart(cartRaw, catalog, kickoff) {
  ...
  for (let item of cart) {
    ...
    if (/detalles del bote|día de bote/i.test(item.name) || /boat details|boat day/i.test(item.name_en)) {
...
function CoverPage({ kickoff, total, lang, editMode, checkinResponses = [] }) {
  ...
  const cityLabel = String(a.city||"").split(",").map(c=>({CTG:"Cartagena",MDE:"Medellín",CDMX:"Ciudad de México",TUL:"Tulum",BOG:"Bogotá"})[c.trim().toUpperCase()]||c.trim()).filter(Boolean).join(" & ");
  const titleLine = a.tripName || cityLabel || "Two Travel Concierge";
      ...
      <div className="cover-body">
        {a.city && (
          <Editable
            ...
            value={`${cityLabel} ${isEs ? "Itinerario Concierge" : "Concierge Itinerary"}`}
          />
        )}
              ...
              <div style={{ padding: "12px 16px", flex: 1 }}>
                ...
                {(a.groupSize || a.checkIn || a.checkOut || a.arrivalDate) && (
                  <div style={{ display: "flex", gap: 16, marginTop: 10, paddingTop: 10, borderTop: "1px solid #f0f0f0", flexWrap: "wrap" }}>
                    ...
                    {a.checkIn && (
                      <div style={{ fontSize: 11, color: "#374151" }}>
                        <span style={{ color: "#aaa", fontSize: 9, textTransform: "uppercase", letterSpacing: 1 }}>Check-in </span>
                        {a.checkIn}
                      </div>
                    )}
                    {a.checkOut && (
                      <div style={{ fontSize: 11, color: "#374151" }}>
                        <span style={{ color: "#aaa", fontSize: 9, textTransform: "uppercase", letterSpacing: 1 }}>Check-out </span>
                        {a.checkOut}
                      </div>
                    )}
                  </div>
      ...
      <div className="ev-content">
        ...
        {timePart && (
          <div className="ev-time-badge">
            ...
            <span style={{ fontWeight:400, color:"#b45309", fontSize:8, letterSpacing:".5px", marginLeft:4 }}>Colombia Standard Time</span>
          </div>
        )}
...
function KeepInTouchPage({ kickoff, page, total }) {
  ...
  const googleReviewUrl = isMexicoCity
    ? "https://www.google.com/maps/place/Two+Travel+Concierge+Mexico+City/@19.4126255,-99.163745,17z/data=!4m8!3m7!1s0x85d1ffe74c93137b:0xa0d5cd7a707b4f62!8m2!3d19.4126255!4d-99.163745!9m1!1b1!16s%2Fg%2F11vj68z9y8?entry=ttu&g_ep=EgoyMDI2MDgxNy4wIKXMDSoASAFQAw%3D%3D"
  ...
  return (
    <div className="root" style={{ padding: "28px 16px 60px", minHeight: "100vh" }}>
      ...
      {(() => {
        ...
        const baseDate = kickoff.arrivalDate || kickoff.arrivalDate2 || "";
        const isoForDay = (di) => {
          if (!baseDate) return "";
